                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <div class="p-3 bg-gray-50 rounded-lg border border-gray-200">
                            <div class="flex items-center">
                                <div class="w-10 h-10 bg-gradient-to-r from-iri-primary to-iri-secondary rounded-full flex items-center justify-center text-white font-semibold mr-3">
                                    {{ strtoupper(substr($actualite->auteur->name ?? 'U', 0, 1)) }}
                                </div>
                                <div>
                                    <div class="font-medium text-gray-900">{{ $actualite->auteur->name ?? 'Non spécifié' }}</div>
                                    @if($actualite->auteur->email)
                                    <div class="text-sm text-gray-500">{{ $actualite->auteur->email }}</div>
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Date de création par l'auteur -->
                    @if($actualite->auteur->created_at)
                    <div>
                        <label class="block text-sm font-medium text-iri-gray mb-2">
                            <i class="fas fa-user-plus mr-2"></i>Compte créé le
                        </label>
                        <div class="p-3 bg-gray-50 rounded-lg border border-gray-200">
                            <span class="text-gray-700">{{ $actualite->auteur->created_at->format('d/m/Y à H:i') }}</span>
                        </div>
                    </div>
                    @endif
                </div>
        ...
        inputFichier.addEventListener('change', function(e) {
            ...
            if (fichiers.length > 0) {
                ...
                fichiers.forEach((fichier, index) => {
                    ...
                    const tailleOk = fichier.size <= 50 * 1024 * 1024; // 50 Mo
                    ...
                    const itemHtml = `
                        <div class="flex items-start gap-3 p-4 bg-white rounded-lg border-2 ${!estPdf || !tailleOk ? 'border-red-500' : 'border-gray-200'}">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                ${!tailleOk ? '<p class="text-xs text-red-600 mb-0 mt-1"><i class="fas fa-times-circle mr-1"></i>Fichier trop volumineux (max 50 Mo)</p>' : ''}
                            </div>
                            <div class="flex-shrink-0">
