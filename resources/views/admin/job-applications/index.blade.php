                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div class="space-y-2">
                        ...
                        <select name="status" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-iri-primary focus:border-iri-primary transition-colors duration-200 bg-white">
                            ...
                            @foreach(\App\Services\JobApplicationStatusService::getStatusOptionsForFilter() as $value => $label)
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                    <div class="space-y-2">
                        ...
                        <select name="status" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-colors duration-200 bg-white" required>
                            ...
                            @foreach(\App\Services\JobApplicationStatusService::getStatusOptionsForBulkActions() as $value => $label)
                            ...
                            @foreach($applications as $application)
                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="space-y-2">
                                            @php
                                                $config = $application->status_config;
                                            @endphp
                                        ...
                                        <div class="flex items-center justify-end space-x-2">
                                            ...
                                            <div class="relative inline-block text-left" x-data="{ open: false }">
                                                ...
                                                <div x-show="open" 
                                                    ...
                                                    <div class="py-1">
                                                        @foreach(\App\Services\JobApplicationStatusService::getStatusOptionsForActions() as $status => $config)
                                                            <form method="POST" action="{{ route('admin.job-applications.update-status', $application) }}" class="block">
                                                                @csrf
                                                                <input type="hidden" name="status" value="{{ $status }}">
                                                                <button type="submit" class="flex items-center w-full px-4 py-2 text-sm text-gray-700 hover:bg-{{ $config['color'] }}-50 transition-colors duration-200">
                                                                    <i class="fas {{ $config['icon'] }} mr-2 text-{{ $config['color'] }}-500"></i>
                                                                    {{ $config['label'] }}
                                                                </button>
                                                            </form>
                                                        @endforeach
                                                ...
                                                <div class="py-1">
                                                    @foreach(\App\Services\JobApplicationStatusService::getStatusOptionsForActions() as $status => $config)
                                                        <form method="POST" action="{{ route('admin.job-applications.update-status', $application) }}">
                                                            @csrf
                                                            <input type="hidden" name="status" value="{{ $status }}">
                                                            <button type="submit" class="flex items-center w-full px-4 py-2 text-sm text-gray-700 hover:bg-{{ $config['color'] }}-50">
                                                                <i class="fas {{ $config['icon'] }} mr-2 text-{{ $config['color'] }}-500"></i>{{ $config['label'] }}
                                                            </button>
                                                        </form>
                                                    @endforeach
                                                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    bulkForm.addEventListener('submit', function(e) {
        ...
        const action = this.querySelector('select[name="status"]').selectedOptions[0].textContent;
        
        if (!confirm(`Êtes-vous sûr de vouloir appliquer l'action "${action}" à ${count} candidature(s) ?`)) {
            e.preventDefault();
        }
    });
});
