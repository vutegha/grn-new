...
<div>
    ...
    <div class="relative w-full h-screen bg-cover bg-center"
        ...
        <div class="absolute top-0 left-0 right-0 z-20 px-4 sm:px-6 lg:px-8 pt-0 hidden md:block">
            ...
            <div class="bg-black/40 backdrop-blur-sm rounded-lg p-2 shadow-lg">
                <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-2">
                    @if(isset($actualites) && $actualites->count() > 0)
                        @foreach($actualites->take(4) as $index => $actualite)
                            <!-- Masquer selon la taille d'écran : 3 sur md, 4 sur lg+ -->
                            <div class="flex items-center space-x-2 hover:bg-white/10 rounded-lg p-2 transition-all duration-300 min-h-[60px] {{ $index >= 3 ? 'hidden lg:flex' : '' }}">
                                <!-- Image circulaire -->
                            ...
                            <div class="flex-1 min-w-0">
                                <a href="{{ route('site.actualite.show', ['slug' => $actualite->slug]) }}" 
                                   class="text-white hover:text-orange-300 font-medium text-xs md:text-sm transition-colors duration-200 leading-tight">
                                    {{ $actualite->titre }}
                                </a>
                            </div>
                        </div>
    ...
    <section id="aboutus" class="py-20 bg-gradient-to-b from-white via-gray-50 to-white relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-4 relative z-10">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
                ...
                <div class="relative overflow-hidden rounded-3xl shadow-2xl group hover:shadow-3xl transition-all duration-700 transform hover:-translate-y-3 mission-card-bg">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div class="lg:col-span-2">
                    ...
                    <div id="projets_stats" class="mt-16 bg-gradient-to-br from-white via-gray-50 to-blue-50 rounded-2xl p-8 shadow-xl border border-gray-100">
                        ...
                        <!-- KPI Cards -->
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12 max-w-6xl mx-auto">
                            <!-- Total Projets -->
    ...
    <!-- Boutons de partage social flottants pour la page d'accueil -->
    <x-global-social-share 
        ...
        customTitle="Programme Gouvernance des Ressources Naturelles - GRN-UCBC"
        customDescription="Découvrez nos projets, publications et services en matière de gouvernance foncière et développement durable au Congo"
        :analytics="true" />
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (typeof pdfjsLib !== 'undefined') {
        ...
        const canvasList = document.querySelectorAll('canvas[data-pdf-url]');
        
        // Ne charger que si des canvas PDF existent réellement
        if (canvasList.length > 0) {
            canvasList.forEach(canvas => {
                ...
                if (!url || url.trim() === '') {
                    return; // Ignorer les canvas sans URL
                }
                ...
                pdfjsLib.getDocument(url).promise.then(pdf => {
                    ...
                    // Ne logger que si ce n'est pas une erreur 403/404 (fichier non trouvé)
                    if (err.status !== 403 && err.status !== 404) {
                        console.error("Erreur lors du rendu du PDF :", err);
                    }
                    // Afficher une image de fallback seulement si le canvas est visible
